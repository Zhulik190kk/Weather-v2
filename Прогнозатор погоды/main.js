...
form.onsubmit = async function (e) {
	...
	if (data.error) {
		...
		const info = data.current.condition.code;
		...
		const weatherData = {
			...
			condition: data.current.is_day
				? 'Day' //info.languages[23]['day_text']
				: 'Night', //info.languages[23]['night_text'],
			imgPath: `./img/${(data.current.is_day ? 'day' : 'night')}.png`, //imgPath,
		};
